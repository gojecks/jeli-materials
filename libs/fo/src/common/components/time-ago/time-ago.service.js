...
export class TimeAgoService {
    ...
    inWords(datetime) {
        datetime = (Date.now() - datetime.getTime());
    ...
    parse(iso8601) {
        if (typeof iso8601 == 'string'){
            iso8601 = iso8601.trim();
            iso8601 = iso8601.replace(/\.\d+/, "");
            iso8601 = iso8601.replace(/-/, "/").replace(/-/, "/");
            iso8601 = iso8601.replace(/T/, " ").replace(/Z/, " UTC");
            iso8601 = iso8601.replace(/([\+\-]\d\d)\:?(\d\d)/, " $1$2");
        }
